...
const Graphs = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responseKeywords = await axios.get('http://82.165.212.88/data/visualize/keywords');
        const responseCountries = await axios.get('http://82.165.212.88/data/visualize/countries');
        const responseCitedTimes = await axios.get('http://82.165.212.88/data/visualize/cited_times');
        const responseAuthors = await axios.get('http://82.165.212.88/data/visualize/authors'); // Agregamos la nueva solicitud para autores
        const responsePublicationYears = await axios.get('http://82.165.212.88/data/visualize/publication_years'); // Agregamos la nueva solicitud para años de publicación
        const responseAbstractKeywords = await axios.get('http://82.165.212.88/data/visualize/abstract'); // Agregamos la nueva solicitud para resúmenes
